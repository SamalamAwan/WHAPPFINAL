...
export const DamageFindScreen = ({ navigation, route }) => {
  ...
  const sendScan = (barcode) => {
    ...
    if (jwt != null) {
      ...
      return fetch('https://api-veen-e.ewipro.com/v1/products/', data)
        ...
        .then((responseData) => {
          console.log(responseData)
          console.log(responseData.products.length)
          if (responseData.status == false){
            setError(responseData.message)
          }
          else{
            ...
            else if (responseData.products.length == 1){
  ...
  const goToProduct = React.useCallback((product) => {
    ...
    navigation.navigate('Damage Item', { id, barcode, itemCode, itemName, photo, stockQty })
   console.log(product)
  },[navigation])
